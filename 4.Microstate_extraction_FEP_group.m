...
% mi=30;  
% for i=1:length(psy)
% EEG = pop_loadset('filename',EEGFiles(psy(i)).name,'filepath',EEGdir); 
...
% end
% 
% save(strcat('pre332_psy_145_100workspace_mi',num2str(mi),'_rng',num2str(seed)),'-v7.3');
% 
% clear;clc;
% for kk=1:100
% eeglab;
% load('pre332_psy_145_100workspace_mi30_rng0.mat');
% load('M4_tem.mat');
% seed=kk;
% %%
% rng(seed);
...
% saveas(gcf, strcat('psy4_rng',num2str(seed),'.jpg'));
% EEG = pop_saveset( EEG, 'filename',strcat('psy_4_145_100workspace0_min',num2str(mi),'_rng',num2str(seed), '.set'), 'filepath','TEM');   
% end
% 
% clear;
% clc;
% eeglab
% EEGdir = 'F:\4447\TEM\';
% EEGFiles = dir([EEGdir 'psy_4_145_100workspace0_min*.set']);
% load('M4_tem.mat');
...
seed=kk;
rng(seed);
EEG = pop_loadset('filename',strcat('sortABCD_psy_4_145_100workspace0_min',num2str(mi),'_rng',num2str(seed), '.set'),'filepath','TEM');  
[ALLEEG, EEG, CURRENTSET] = eeg_store( ALLEEG, EEG, 0 );
eeglab redraw;
...
writematrix(feature1, strcat('sortABCD_psy_4_145_100workspace0_min',num2str(mi),'_rng',num2str(seed), '_feature_classic.txt'));
...
writetable(SUB4, strcat('psy_100RNA4447_145_4M_s',num2str(mi),'_rng',num2str(seed),'.txt'));
