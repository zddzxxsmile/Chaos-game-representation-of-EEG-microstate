...
% end
% 
% save(strcat('pre332_145_100workspace_mi',num2str(mi),'_rng',num2str(seed)),'-v7.3');  %% save
% 
% clear;clc;
% for kk=1:100   %random
% eeglab;
...
% %saveas(gcf, strcat('all4_rng',num2str(seed),'.jpg'));
% EEG = pop_saveset( EEG, 'filename',strcat('all_4_145_100workspace0_min',num2str(mi),'_rng',num2str(seed), '.set'), 'filepath','TEM');  
% end
% 
% %% sorting microstates
% clear;
...
% eeglab
% EEGdir = 'F:\4447\TEM\';
% EEGFiles = dir([EEGdir 'all_4_145_100workspace0_min*.set']);
% load('M4_tem.mat');
% for i=1:length(EEGFiles)
%    
%    
...
seed=kk;
rng(seed);
EEG = pop_loadset('filename',strcat('sortABCD_all_4_145_100workspace0_min',num2str(mi),'_rng',num2str(seed), '.set'),'filepath','TEM'); 
[ALLEEG, EEG, CURRENTSET] = eeg_store( ALLEEG, EEG, 0 );
eeglab redraw;    
...
writematrix(feature1, strcat('sortABCD_all_4_145_100workspace0_min',num2str(mi),'_rng',num2str(seed), '_feature_classic.txt'));
...
writetable(SUB4, strcat('all_100RNA4447_145_4M_s',num2str(mi),'_rng',num2str(seed),'.txt'));
